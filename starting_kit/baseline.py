...
from keras.layers import Dropout, Dense, Embedding, LSTM, Conv1D, MaxPooling1D, Activation, Bidirectional, Input
...
def buildModel(embeddingMatrix):
    ...
    embedded_sequences_1 = embeddingLayer(sequence_1_input)
    x1 = lstm_layer(embedded_sequences_1)

    # Creating LSTM Encoder layer for Second Sentence
    sequence_2_input = Input(shape=(MAX_SEQUENCE_LENGTH,), dtype='int32')
    embedded_sequences_2 = embeddingLayer(sequence_2_input)
...
#     leaks_dense = Dense(self.number_dense_units/2, activation=self.activation_function)(leaks_input)
    ...
    merged = Dense(50, activation='sigmoid')(merged)
    merged = BatchNormalization()(merged)
    merged = Dropout(0.1)(merged)
...
#     model.save('EP%d_LR%de-5_LDim%d_BS%d.h5_SiameseNetwork'%(NUM_EPOCHS, int(LEARNING_RATE*(10**5)), LSTM_DIM, BATCH_SIZE))
    model = load_model('EP%d_LR%de-5_LDim%d_BS%d.h5_SiameseNetwork'%(NUM_EPOCHS, int(LEARNING_RATE*(10**5)), LSTM_DIM, BATCH_SIZE))
    ...
    print "Learning rate : %.3f, LSTM Dim : %d, Dropout : %.3f, Batch_size : %d" % (LEARNING_RATE, LSTM_DIM, DROPOUT, BATCH_SIZE)
